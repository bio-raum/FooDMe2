...
if wget --tries 3 --quiet --no-check-certificate $PROXY_OPTIONS ${BLAST} ; then :
else
...
# Get Readme
wget --tries 3 --quiet --no-check-certificate $PROXY_OPTIONS  -O README.html ${BLAST}README
...
while IFS=$'\t' read -r part md5; do
  ...
  wget --tries 3 --quiet --no-check-certificate $PROXY_OPTIONS -N   $md5
  ...
  if [ -f $(basename ${part}) ]; then
    ...
    if [ $? -ne 0 ]; then
      ...
      rm $(basename ${part})
      wget --tries 3 --quiet --no-check-certificate $PROXY_OPTIONS  $part
      md5sum  -c $(basename ${md5})
