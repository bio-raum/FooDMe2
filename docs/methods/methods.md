...
![seqconfig](../img/foodme2_sequencing_configs.png)<br>
<b>Figure 1</b>: Effect of sequencing length on amplicon coverage. A: Forward and reverse read overlap and can be merged, B: Forward and reverse read each fully cover the amplicon and will carry primer sites on both ends (3' and 5'), C: Forward and reverse read do not overlap and cannot be merged.
...
This allows merging R1 and R2 sequences relatively easily (Figure 1, A) and working on full sequences for the BLAST search. If for any readon one decides to work with paired 150bp sequences,
this has the dramatic effect of not allowing an overlap-based read merging (Figure 1, C), which in turns has several effects on downstream parameters choices.
